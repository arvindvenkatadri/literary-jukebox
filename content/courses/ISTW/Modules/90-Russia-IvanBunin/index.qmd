...
knitr::include_graphics("bunin.jpg")
...
{{< pdf "../../../../materials/pdfs/IvanBunin-Sunstroke.pdf" >}}
...
box <- c(xmin = 30, xmax = 45, ymin = 30, ymax = 51)
bbox <- matrix(box, byrow = FALSE, nrow = 2, ncol = 2, dimnames = list(c("x", "y"), c("min", "max")))

russian_rivers <- World_rivers %>%
  filter(name %in% c("Volga", "Don", "Dnipro")) %>% 
  sf::st_set_crs(crs) %>% 
  sf::st_crop(box)
...
bunin_cities <-
  maps::world.cities %>% 
  filter(country.etc %in% c("Russia", "Ukraine" ),
    name %in% c(
      "Moscow",
      "Samara",
      "Volgograd",
...
ggplot() + 
  geom_sf(data = bunin_world) + 
  geom_sf(data = russian_rivers, aes(color = name)) + 
  geom_sf(data = bunin_cities, color = "red", size = 2) + 
  theme_void() + 
  labs(title = "The World from Bunin's Sunstroke", 
       subtitle = "The World, the Rivers, and the Cities") + 
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 20), plot.subtitle = element_text(hjust = 0.5, face = "italic", size = 16), legend.position = "none")
```
...
#| echo: false
#| 
leaflet(data = bunin_world,height = "50%",width = "50%",) %>% 
  leaflet() %>%
  ...
  addScaleBar(
    ...
    fitBounds(lat1 = 41, lat2 = 51, lng1 = 35, lng2 = 45)
...
::: {#fig-torchballad}
<center>
{{< video https://youtu.be/Cq7Je8uveKE width="560" height="400" align="center" title="Take a Look at Me Now" >}}
</center>
...
::: {#fig-torchballad}
<center>
{{< video https://youtu.be/s24reWXydp8 width="560" height="400" align="center" title="Vacation" >}}
</center>
